...
>(type: S['type'], ...args: string[]) => Fn<T, S>

export type NamedCreatorFn = <
  T extends T[number][],
  S extends CommonAction = Updatable<CommonActions<Compare<T[number], CommonInstance>>, 'type' | 'name'>,
>(name: string, type: S['type'], ...args: string[]) => Fn<T, S>

export function toObjects<T extends T[number][]>(payload: T, arg: string, i: number): T[number] {
  return { [arg]: payload[i] }
}

export const makeNamedAction: NamedCreatorFn = (name, type, ...args) => (...payload) => Object
  .assign({ type, name }, ...args
    .map(toObjects.bind(null, payload)))

export const makeAction: CreatorFn = (type, ...args) => (...payload) => Object
  .assign({ type }, ...args
    .map(toObjects.bind(null, payload)))